...
class DatabaseSeeder extends Seeder
{
    ...
    public function run(): void
    {
        ...
        $this->command->info('Seeded admin, two providers (one pending), two applicants (one incomplete), demo listings, applications in three statuses, notifications, and audit history.');
    }
     ...
     * for this account and the profile checklist has three open items - the
     * state Phase 13 of a viva needs to show "what an unfinished profile looks
     * like" without narrating it from an empty database.
     *
     * The certificate is advisory, not enforced: nothing in ApplicationService
     * blocks submission on it, and this account can apply exactly as freely as
     * the complete one. What it demonstrates is the checklist and the provider
     * review page's "no certificate on file" state, not a rejected submission -
     * see docs/user-guide.md for why the flow does not gate on this.
     */
    private function applicantWithoutCertificate(): User
    {
        ...
        ApplicantProfile::updateOrCreate(
            ['user_id' => $applicant->user_id],
            [
                'education_level' => 'High School (A-Level)',
    ...
    private function opportunities(User $provider): void
    {
        ...
        // demonstrating a weighted score. Restricted to Manicaland: Chipo (seeded
        // there) meets it, Tendai (Harare) does not, which is the pairing that makes
        // the difference visible without narrating it.
        Opportunity::where('title', 'Rural Schools A-Level Support Fund')
            ->update(['required_province' => 'Manicaland']);

        // A second, distinct hard rule: a listing that requires a results
        // certificate on file. Chipo (seeded without one) shows "Requirements
        // not met" here specifically because of the missing document, which is
        // the one demonstration that ties her incomplete profile to a visible
        // consequence rather than only a checklist badge. Tendai has the
        // certificate, so his score for this listing is unaffected.
